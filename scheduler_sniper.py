...
# 🚨 MODIFIED: [제1헌법 준수] send_order, cancel_order, get_unfilled_orders_detail 등 모든 KIS 외부 통신에 누락되었던 wait_for(timeout=10.0~15.0) 타임아웃 족쇄 100% 전면 결속
...
# 🚨 MODIFIED: [Insight 06/07] JSON Iterable 결측치(None) 유입 시 발생하는 TypeError 붕괴 방어용 `or []`, `or {}` 단락 평가 쉴드 주입
# 🚨 MODIFIED: [Case 26 절대 헌법 준수] 텔레그램 HTML 파서 붕괴 방어용 html.escape 쉴드 전역 강제 주입
# ==========================================================
import logging
import datetime
...
async def scheduled_sniper_monitor(context):
    ...
    async def _do_sniper():
        async with tx_lock:
            ...
            for t in active_tickers:
                try:
                    ...
                    if version == "V_REV":
                        ...
                        if q_ledger:
                            q_data = (await asyncio.to_thread(q_ledger.get_queue, t)) or []
                            ...
                            if actual_qty == 0 and total_q > 0:
                                ...
                                dump_jitter_sec = int(_safe_float(tracking_cache.get(f"AVWAP_DUMP_JITTER_{t}", 0)))
                    ...
                    if version == "V_REV" and await asyncio.to_thread(getattr(cfg, 'get_avwap_hybrid_mode', lambda x: False), t):
                        if not tracking_cache.get(f"AVWAP_INIT_{t}"):
                            try:
                                ...
                                if saved_state:
                                    tracking_cache[f"AVWAP_BOUGHT_{t}"] = bool(saved_state.get('bought', False))
                                    tracking_cache[f"AVWAP_SHUTDOWN_{t}"] = bool(saved_state.get('shutdown', False))
                                    tracking_cache[f"AVWAP_EXECUTED_BUY_{t}"] = bool(saved_state.get('executed_buy', False))
                                    tracking_cache[f"AVWAP_QTY_{t}"] = int(_safe_float(saved_state.get('qty', 0)))
                                    tracking_cache[f"AVWAP_AVG_{t}"] = _safe_float(saved_state.get('avg_price', 0.0))
                                    tracking_cache[f"AVWAP_STRIKES_{t}"] = int(_safe_float(saved_state.get('strikes', 0)))
                                    tracking_cache[f"AVWAP_DAILY_BOUGHT_{t}"] = int(_safe_float(saved_state.get('daily_bought_qty', 0)))
                                    tracking_cache[f"AVWAP_DAILY_SOLD_{t}"] = int(_safe_float(saved_state.get('daily_sold_qty', 0)))
                                    tracking_cache[f"AVWAP_TRAP_ODNO_{t}"] = str(saved_state.get('trap_odno', ""))
                                    tracking_cache[f"AVWAP_PM_H_{t}"] = _safe_float(saved_state.get('PM_H', 0.0))
                                    tracking_cache[f"AVWAP_PM_L_{t}"] = _safe_float(saved_state.get('PM_L', 0.0))
                                    tracking_cache[f"AVWAP_T_H_{t}"] = _safe_float(saved_state.get('T_H', 0.0))
                                    tracking_cache[f"AVWAP_T_L_{t}"] = _safe_float(saved_state.get('T_L', 0.0))
                                    tracking_cache[f"AVWAP_OFFSET_{t}"] = _safe_float(saved_state.get('offset', 0.0))
                                    tracking_cache[f"AVWAP_DUMP_JITTER_{t}"] = int(_safe_float(saved_state.get('dump_jitter_sec', 0)))
                                    tracking_cache[f"AVWAP_LIMIT_ORDER_PLACED_{t}"] = bool(saved_state.get('limit_order_placed', False))
                                    tracking_cache[f"AVWAP_PLACED_TARGET_TH_{t}"] = _safe_float(saved_state.get('placed_target_th', 0.0))
                                    tracking_cache[f"AVWAP_BUY_ODNO_{t}"] = str(saved_state.get('buy_odno', ""))
                                    tracking_cache[f"AVWAP_TRAP_PLACED_TIME_{t}"] = str(saved_state.get('trap_placed_time', ""))
                            except Exception: pass
                        ...
                        avwap_qty = int(_safe_float(tracking_cache.get(f"AVWAP_QTY_{t}", 0)))
                        avwap_avg = _safe_float(tracking_cache.get(f"AVWAP_AVG_{t}", 0.0))
                        ...
                        base_day_open = _safe_float(tracking_cache.get(f"AVWAP_DAY_OPEN_{target_base}", 0.0))
                        ...
                        for attempt in range(3):
                            try:
                                ...
                                prev_c = _safe_float(res_batch[0]) if not isinstance(res_batch[0], Exception) and res_batch[0] else 0.0
                                amp5 = _safe_float(res_batch[1]) if not isinstance(res_batch[1], Exception) and res_batch[1] else 0.0
                                ...
                                ma_5day = _safe_float(res_batch[4]) if len(res_batch) > 4 and not isinstance(res_batch[4], Exception) and res_batch[4] else 0.0
                        ...
                        avwap_state_dict = {
                            "strikes": int(_safe_float(tracking_cache.get(f"AVWAP_STRIKES_{t}", 0))),
                            "shutdown": bool(tracking_cache.get(f"AVWAP_SHUTDOWN_{t}", False)),
                            "executed_buy": bool(tracking_cache.get(f"AVWAP_EXECUTED_BUY_{t}", False)),
                            "qty": avwap_qty,
                            "avg_price": avwap_avg,
                            "bought": bool(tracking_cache.get(f"AVWAP_BOUGHT_{t}", False)),
                            "daily_bought_qty": int(_safe_float(tracking_cache.get(f"AVWAP_DAILY_BOUGHT_{t}", 0))),
                            "daily_sold_qty": int(_safe_float(tracking_cache.get(f"AVWAP_DAILY_SOLD_{t}", 0))),
                            "trap_odno": str(tracking_cache.get(f"AVWAP_TRAP_ODNO_{t}", "")),
                            "buy_odno": str(tracking_cache.get(f"AVWAP_BUY_ODNO_{t}", "")),
                            "PM_H": _safe_float(tracking_cache.get(f"AVWAP_PM_H_{t}", 0.0)),
                            "PM_L": _safe_float(tracking_cache.get(f"AVWAP_PM_L_{t}", 0.0)),
                            "T_H": _safe_float(tracking_cache.get(f"AVWAP_T_H_{t}", 0.0)),
                            "T_L": _safe_float(tracking_cache.get(f"AVWAP_T_L_{t}", 0.0)),
                            "offset": _safe_float(tracking_cache.get(f"AVWAP_OFFSET_{t}", 0.0)),
                            "limit_order_placed": bool(tracking_cache.get(f"AVWAP_LIMIT_ORDER_PLACED_{t}", False)),
                            "placed_target_th": _safe_float(tracking_cache.get(f"AVWAP_PLACED_TARGET_TH_{t}", 0.0)),
                            "dump_jitter_sec": int(_safe_float(tracking_cache.get(f"AVWAP_DUMP_JITTER_{t}", 0))),
                            "trap_placed_time": str(tracking_cache.get(f"AVWAP_TRAP_PLACED_TIME_{t}", ""))
                        }
                        ...
                        tracking_cache[f"AVWAP_PM_H_{t}"] = _safe_float(decision.get("PM_H", tracking_cache.get(f"AVWAP_PM_H_{t}", 0.0)))
                        tracking_cache[f"AVWAP_PM_L_{t}"] = _safe_float(decision.get("PM_L", tracking_cache.get(f"AVWAP_PM_L_{t}", 0.0)))
                        tracking_cache[f"AVWAP_T_H_{t}"] = _safe_float(decision.get("T_H", tracking_cache.get(f"AVWAP_T_H_{t}", 0.0)))
                        tracking_cache[f"AVWAP_T_L_{t}"] = _safe_float(decision.get("T_L", tracking_cache.get(f"AVWAP_T_L_{t}", 0.0)))
                        tracking_cache[f"AVWAP_OFFSET_{t}"] = _safe_float(decision.get("offset", tracking_cache.get(f"AVWAP_OFFSET_{t}", 0.0)))
                        
                        if decision.get("limit_order_placed") is not None:
                            tracking_cache[f"AVWAP_LIMIT_ORDER_PLACED_{t}"] = bool(decision.get("limit_order_placed"))
                        if decision.get("placed_target_th") is not None:
                             tracking_cache[f"AVWAP_PLACED_TARGET_TH_{t}"] = _safe_float(decision.get("placed_target_th"))
                        if decision.get("trap_placed_time") is not None:
                             tracking_cache[f"AVWAP_TRAP_PLACED_TIME_{t}"] = str(decision.get("trap_placed_time"))
                        ...
                        if action == "PLACE_TRAP":
                            ...
                            if qty > 0 and price > 0:
                                ...
                                try:
                                    res = await asyncio.wait_for(
                                        asyncio.to_thread(broker.send_order, t, "BUY", qty, exec_price, "LIMIT"),
                                        timeout=15.0
                                    )
                                except Exception as e:
                                    logging.error(f"🚨 [{t}] AVWAP 덫 장전 통신 에러: {e}")
                                    res = None
                                ...
                                if safe_res.get('rt_cd') == '0' and odno:
                                    ...
                                    state_data['trap_placed_time'] = str(decision.get("trap_placed_time", ""))
                                    await asyncio.to_thread(strategy.v_avwap_plugin.save_state, t, now_est, state_data)
                                else:
                                    err_msg = html.escape(str(safe_res.get('msg1') or '응답 없음'))
                            ...
                            if buy_odno:
                                ...
                                if ccld_qty > 0:
                                    ...
                                    old_qty = int(_safe_float(tracking_cache.get(f"AVWAP_QTY_{t}", 0)))
                                    old_avg = _safe_float(tracking_cache.get(f"AVWAP_AVG_{t}", 0.0))
                                    ...
                                    daily_b = int(_safe_float(tracking_cache.get(f"AVWAP_DAILY_BOUGHT_{t}", 0))) + ccld_qty
                                    ...
                                    try:
                                        ...
                                        logging.error(f"🚨 [{t}] AVWAP 익절 덫 장전 통신 에러: {e}")
                                        trap_res = None
                                        
                                    safe_trap_res = trap_res if isinstance(trap_res, dict) else {}
                                    trap_odno = str(safe_trap_res.get('odno') or '')
                                    
                                    if safe_trap_res.get('rt_cd') == '0' and trap_odno:
                                        tracking_cache[f"AVWAP_TRAP_ODNO_{t}"] = trap_odno
                                        msg += f"\n\n🎯 <b>[투트랙 엑시트 장전]</b>\n▫️ +2.0% 수익 타점(<b>${trap_price:.2f}</b>)에 익절 덫을 즉시 자동 장전했습니다."
                                    else:
                                        trap_err = html.escape(str(safe_trap_res.get('msg1') or '오류'))
                            ...
                            if qty > 0:
                                ...
                                except Exception: exec_hist = []
                                safe_exec_hist = exec_hist if isinstance(exec_hist, list) else []
                                
                                if trap_odno and safe_exec_hist:
                                    trap_filled_qty = sum(int(_safe_float(ex.get('ft_ccld_qty'))) for ex in safe_exec_hist if isinstance(ex, dict) and str(ex.get('odno')) == trap_odno)
                                ...
                                if remaining_qty > 0:
                                    ...
                                    try:
                                        ...
                                        logging.error(f"🚨 [{t}] AVWAP 덤핑 장전 통신 에러: {e}")
                                        res = None
                                ...
                                if total_sold > 0:
                                    ...
                                    shutdown_flag = bool(tracking_cache.get(f"AVWAP_SHUTDOWN_{t}", False))
                                    
                                    if new_qty == 0:
                                        strikes = int(_safe_float(tracking_cache.get(f"AVWAP_STRIKES_{t}", 0))) + 1
                                        ...
                                        shutdown_flag = True
                                        new_avg = _safe_float(tracking_cache.get(f"AVWAP_AVG_{t}", 0.0))
    
                                    try:
                                        await context.bot.send_message(chat_id=chat_id, text=msg, parse_mode='HTML')
                                    except Exception: pass
                       
                                    daily_s = int(_safe_float(tracking_cache.get(f"AVWAP_DAILY_SOLD_{t}", 0))) + total_sold
                ...
                master_switch = str(await asyncio.to_thread(getattr(cfg, 'get_master_switch', lambda x: "ALL"), t))
                sniper_buy_locked = bool(await asyncio.to_thread(getattr(cfg, 'get_sniper_buy_locked', lambda x: False), t))
                sniper_sell_locked = bool(await asyncio.to_thread(getattr(cfg, 'get_sniper_sell_locked', lambda x: False), t))
                ...
                if sniper_func: 
                    ...
                    res = {"action": "HOLD", "reason": "스나이퍼 모듈 누락(Bypass)", "limit_price": 0.0, "qty": 0}
                    
                res = res if isinstance(res, dict) else {}
                action = str(res.get("action", "HOLD"))
                reason = str(res.get("reason", ""))
                limit_p = _safe_float(res.get("limit_price"))
                ...
                if action == "BUY" and not is_rev and not sniper_buy_locked and master_switch != "UP_ONLY":
                    qty = int(_safe_float(res.get("qty", 0)))
                    if qty > 0:
                        try:
                        ...
                        try:
                            ...
                            logging.error(f"🚨 [{t}] 스나이퍼 매수 장전 통신 에러: {e}")
                            order_res = None
                            
                        safe_order_res = order_res if isinstance(order_res, dict) else {}
                        odno = str(safe_order_res.get('odno') or '')
                        
                        if safe_order_res.get('rt_cd') == '0' and odno:
                            ccld_qty = 0
                            for _ in range(4):
                                 await asyncio.sleep(2.0)
                                 try:
                            ...
                            if ccld_qty > 0:
                                ...
                                actual_exec_price = next((_safe_float(ex.get('ft_ccld_unpr3')) for ex in safe_exec_history if isinstance(ex, dict) and str(ex.get('sll_buy_dvsn_cd')) == '02' and str(ex.get('odno')) == odno and _safe_float(ex.get('ft_ccld_unpr3')) > 0), next((_safe_float(ex.get('ft_ccld_unpr3')) for ex in safe_exec_history if isinstance(ex, dict) and str(ex.get('sll_buy_dvsn_cd')) == '02' and _safe_float(ex.get('ft_ccld_unpr3')) > 0), limit_p))
                            ...
                            err_msg = html.escape(str(safe_order_res.get('msg1') or '응답 없음'))
                ...
                if is_upward_active and action in ["SELL_QUARTER", "SELL_JACKPOT"]:
                    ...
                    if qty > 0:
                        ...
                        try:
                            ...
                            logging.error(f"🚨 [{t}] 스나이퍼 매도 장전 통신 에러: {e}")
                            order_res = None
                        ...
                        if safe_order_res.get('rt_cd') == '0' and odno:
                            ...
                            if ccld_qty > 0:
                                ...
                                actual_exec_price = next((_safe_float(ex.get('ft_ccld_unpr3')) for ex in safe_exec_history if isinstance(ex, dict) and str(ex.get('sll_buy_dvsn_cd')) == '01' and str(ex.get('odno')) == odno and _safe_float(ex.get('ft_ccld_unpr3')) > 0), next((_safe_float(ex.get('ft_ccld_unpr3')) for ex in safe_exec_history if isinstance(ex, dict) and str(ex.get('sll_buy_dvsn_cd')) == '01' and _safe_float(ex.get('ft_ccld_unpr3')) > 0), limit_p))
                                display_price = actual_exec_price if actual_exec_price > 0 else limit_p
            ...
            logging.error(f"🚨 [{t}] 스나이퍼 메인 루프 에러 (Cascade 방어): {e}", exc_info=True)
